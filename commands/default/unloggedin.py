...
These are copied from Evennia's default, in case I need to overload them later.
"""
import re
import datetime
    ...
    def func(self):
        ...
        if len(parts) != 2:
            string = (
                ...
                "\nAn account name is not a character name. Account names may not overlap"
                "\nwith any existing characters. For best results, choose a unique handle."
            )
            session.msg(string)
            return
